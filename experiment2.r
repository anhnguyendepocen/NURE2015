...
for (i in 1:years){                           #do this once for each grade
  if(i==2){
    h=seq(from=1, to=nstu)
    for(p in 1:nstu)                          #from student 1 to the last student
    {
      if(p%%3 == 1)                           #if the student number mod 3 is 1
        theta[p]=h[p]+.2                      #add .2 to theta
      if (p%%3 == 2)                          #if the student number mod 3 is 2
        theta[p]=h[p]                         #leave theta alone
      if(p%%3 == 0)                           #if the student number mod 3 is 0
        theta[p]=h[p]-.2                      #subtract .2 from theta
    } 
  }
