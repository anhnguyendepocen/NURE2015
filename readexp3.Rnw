...
files=list.files("experiment3")                     #list files in target subdirectory
n=length(files)                                     #count of files 
n=2
print(n)
#
for (i in 1:n){                                     #read each file and extract SGP values
  fname=paste("experiment3/",files[[i]],sep="")     #file to load
  load(fname)
...
theta=thetas$theta
df3=cbind(df2,theta)
p=ggplot(df2,aes(x=theta,y=means))
p+geom_point(color=levels,size=0.5)
@
<<>>=
library(ggplot2)
load("Rdata/samplen70.Rdata")
theta=thetas$theta
df3=cbind(df2,theta)
p=ggplot(df2,aes(x=theta,y=sds))
p+geom_point(color=levels,size=0.5)
@
